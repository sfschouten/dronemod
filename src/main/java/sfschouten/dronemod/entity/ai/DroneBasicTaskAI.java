...
public class DroneBasicTaskAI extends EntityAIBase {
	...
	public void startExecuting() {
		...
		TileEntityMarker currentWork = r.marker;
		if(currentRadius == -1){
	...
	public boolean executeAI(EntityDrone e) {
		if (!e.getWorkMarkers().isEmpty()) {
			...
			if (!(currentX + 0.2 < e.posX && currentX + 0.8 > e.posX) || !(currentZ + 0.2 < e.posZ && currentZ + 0.8 > e.posZ)
					|| !(currentY + 1.2 < e.posY && currentY + 1.8 > e.posY)) {
				// Drone has not arrived where he needs to be so move towards
				// it.
				...
				if (currentBlock < cf) {
					...
					if (currentRadius > 1) {
						...
						if (e.getModules().get(0).hasDepth()) {
							currentDepth++;
						} else {
